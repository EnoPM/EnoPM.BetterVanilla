...
public static class MeetingHudExtensions
{
    ...
    {
        if (meetingHud.state is MeetingHud.MeetingStates.Results or MeetingHud.MeetingStates.Proceeding)
        {
    ...
    private static IEnumerator CoRandomizeVoteAreaPositions(this MeetingHud meetingHud, List<byte> ids)
    {
        ...
        var positions = aliveVoteAreas
            .OrderBy(x => ids.IndexOf(x.VotedForId.Value))
    ...
    {
        var votedPva = vote.Voted != null ? meetingHud.playerStates.FirstOrDefault(x => x != null && x.VotedForId.Value == vote.Voted.Player.PlayerId) : null;
